//! Series sizing — reference-matching candlestick sizing algorithms.
//!
//! Used by GeometryGenerator to compute pixel-exact candle dimensions.

use crate::core::viewport::Viewport;

// ── reference-matching candlestick sizing (pixel-exact) ─────────────────────────

/// Matches reference implementation `optimalCandlestickWidth(barSpacing, pixelRatio)`.
...
/// Wick width in physical pixels matching reference implementation.
...
/// Border width in physical pixels matching reference implementation.
...
/// Ensure bar_width parity matches wick_width parity (reference trick for symmetry).
...
impl CandleSizing {
    ...
    pub fn compute_from_pane(pane_w: f64, vp: &Viewport, h_ratio: f64, v_ratio: f64) -> Self {
        ...
        // Horizontal sizing uses h_ratio (matches the reference implementation's horizontalPixelRatio)
...
mod tests {
    ...
    fn reference_optimal_candlestick_width(bar_spacing: f64, pixel_ratio: f64) -> f64 {
    ...
    #[test]
    fn optimal_width_matches_reference_formula_across_zoom_presets() {
        ...
        for spacing in bar_spacings {
            for ratio in pixel_ratios {
                let expected = reference_optimal_candlestick_width(spacing, ratio);
    ...
    #[test]
    fn wick_and_border_widths_are_reference_compatible() {
