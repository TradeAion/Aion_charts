...
const out = { initial: await w() };
await p.evaluate(() => window.__reference.chart.applyOptions({ leftPriceScale: { borderColor: "#FF0000" } }));
await raf2();
out.after_border_color = await w();
await p.evaluate(() => window.__reference.chart.applyOptions({ crosshair: { vertLine: { color: "#00FF00" } } }));
await raf2();
out.after_crosshair_color = await w();
